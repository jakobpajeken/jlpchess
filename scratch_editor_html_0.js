...
function createGameWidget(container, game){
  ...
  function renderMoveList(){
    listEl.innerHTML = '';
    for(var i = 0; i < historyVerbose.length; i += 2){
...
function openGameForm(idx){
  ...
  if(copyFromSelect){
    copyFromSelect.addEventListener('change', function(){
      ...
      document.getElementById('gf-pgn').value = entry.pgn;
...
function renderBlog(){
  ...
  blogPosts.forEach(function(p, idx){
    ...
    row.innerHTML =
      ...
      '<div class="item-info"><div class="item-title">' + escapeHtml(previewLang(p.title) || '(ohne Titel)') + (badge ? ' ' + badge : '') + (finishedBadge ? ' ' + finishedBadge : '') + (guestBadge ? ' ' + guestBadge : '') + (lockBadge ? ' ' + lockBadge : '') + '</div>' +
      ...
      (notYetLive && p.slug ? '<button type="button" class="btn small" data-toggle-guest-editable="' + idx + '">' + (p.guestEditable ? 'Freigabe aufheben' : 'Für Gast-Editor freigeben') + '</button>' : '') +
...
async function toggleGuestEditable(idx){
  ...
  if(!post.guestEditable) post.editLock = null;
  try{
...
function openBlogForm(idx){
  ...
  slot.innerHTML =
    ...
    '<div class="field"><label>Text (Deutsch)</label><span class="hint">Wird Absatz für Absatz automatisch übersetzt (Bilder/Partien bleiben an der gleichen Stelle) – danach frei bearbeitbar. Markierten Text mit <strong>F</strong>/<em>K</em>/<u>U</u> fett, kursiv bzw. unterstrichen machen.</span>' +
    ...
    '<button type="button" class="btn small" id="bf-ai-btn-de" title="Markierten Text (oder alles) von der KI überarbeiten lassen">✨ KI-Hilfe</button></div>' +
    ...
    '<span class="hint">Hier Partien hinzufügen, dann mit „In Text einfügen“ an der gewünschten Stelle im englischen Text platzieren. Mehrere Partien ankreuzen und „Auswahl als ein Brett einfügen“ klicken, um sie wie auf der Karriere-Seite in einem gemeinsamen Brett mit Auswahlmenü darzustellen.</span>' +
  ...
  /* ---- inline image insertion into the (English) body text ---- */
  ...
  /* ---- inline link insertion into the (English) body text – e.g. for
  ...
  document.getElementById('bf-insert-link').addEventListener('click', function(){
  ...
  function openPostGameForm(gIdx){
    ...
    if(copyFromSelect){
      copyFromSelect.addEventListener('change', function(){
        ...
        document.getElementById('bfg-pgn').value = entry.pgn;
        copyFromSelect.value = '';
        showToast('Übernommen – unten anpassen und speichern ✓');
      });
    }
    ...
    function runPreview(){
      var pgnVal = document.getElementById('bfg-pgn').value.trim();
    ...
    if(g.pgn) runPreview(); /* editing an existing game – show it right away */
